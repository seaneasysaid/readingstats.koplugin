...
> 菜单进入，无需手势；阅读报告 + 日历热力图 + GitHub 风格全年贡献图 + 阅读分析，四视图并存。

**「轻量版」是什么意思**：相对 KOReader 内置的 **「阅读统计」** 插件，本插件只做
「看图」这一件事 —— 不接管阅读器统计逻辑、不改阅读界面、不加手势、不常驻后台。
它只是在菜单里多一个入口，点开就读 `statistics.sqlite3` 画图，关掉即退出。
...
| 视图 | 说明 |
| --- | --- |
| **阅读报告** | 全屏卡片视图（**仿微信读书风格**）：**周 / 月 / 年 / 总** 四周期切换，概览（时长 / 天数 / 日均 / 较上期）+ 阅读时长趋势柱图 + 读书排行 Top10 + 阅读偏好。底部 `上一周期 / 下一周期` 翻页。 |
...
<img src="docs/screenshots/01-menu.png" alt="菜单" width="520" />

层级：`☰ 工具` → `阅读足迹` → `阅读报告 / 日历阅读统计 / 阅读热力图 / 阅读分析 / 设置`

### ① 阅读报告（全屏 · 本月 · 仿微信读书）

> 卡片式布局、周期切换与各项统计的**视觉风格**参考微信读书（WeRead）的「阅读统计」；数据完全来自本地 `statistics.sqlite3`，代码独立实现（本项目为 MIT，故未使用 WeRead 插件的代码或云端数据）。

顶部标题居中（如 `本月·2026年09月`），下面一排 **周 / 月 / 年 / 总** 周期切换条；主体为四张卡片：**概览 / 阅读时长趋势 / 读书排行 / 阅读偏好**；底部为 `上一周期 / 下一周期`。

<img src="docs/screenshots/09-report-month.png" alt="阅读报告 · 本月" width="320" />

### ② 日历阅读统计

<img src="docs/screenshots/02-calendar.png" alt="日历阅读统计" width="320" />

### ③ 阅读热力图 · 26 周（单行铺满）

<img src="docs/screenshots/03-heatmap-26w.png" alt="阅读热力图 26 周" width="520" />

### ③ 阅读热力图 · 52 周（拆成两行，每行 26 周）

<img src="docs/screenshots/04-heatmap-52w.png" alt="阅读热力图 52 周" width="460" />

### ④ 阅读分析

红框 = 可点击的地方，编号对应下方图例。

<img src="docs/screenshots/05-insights.png" alt="阅读分析" width="460" />

### ④-1 年度书籍排行（点「books read」进入）

<img src="docs/screenshots/07-booklist-year.png" alt="年度书籍排行" width="320" />

### ④-2 月度书籍排行（点某根月份柱进入）

<img src="docs/screenshots/08-booklist-month.png" alt="月度书籍排行" width="320" />

### 设置

<img src="docs/screenshots/06-settings.png" alt="设置" width="460" />
...
| 位置 | 操作 | 结果 |
| --- | --- | --- |
| 顶部 **周 / 月 / 年 / 总** | 点击 | 切换统计周期（本周 / 本月 / 本年 / 累计） |
| 右上角 **关闭** | 点击 | 退出阅读报告 |
| 底部 **上一周期 / 下一周期** | 点击 | 向前 / 向后翻一个周期（到顶 / 到底时置灰） |
| 卡片区 | 上下滑动 | 滚动查看四张卡片（概览 / 趋势 / 排行 / 偏好） |
...
1. 下载仓库里的 `readingstats.koplugin/` 整个目录（6 个 `.lua` 文件），后续步骤同上。
...
- **5 级灰阶**：`168 / 122 / 76 / 25`，在墨水屏上实测过对比度，层次拉得开。
...
├── _meta.lua               # 插件元信息
├── reading_stats_view.lua  # ① 阅读报告（全屏卡片）
├── calendar_stats.lua      # ② 日历阅读统计
├── heatmap_view.lua        # ③ 阅读热力图
